...
本文定义 Locus Core 如何把规范声明视图、调用现场和现实证据组合成可解释的 Resolve / Probe 结果。Workspace 负责发现、加载和组合 Scope；Core 从已校验的 Effective View 开始，编排 Resolve、Provider 和 Observation。

- 用户和 Agent 依赖的稳定接口见[公共契约](contracts/README.md)
- 核心产品语义见[产品设计](产品设计.md)
- Locus Home、Registry Source、Scope 组合和本地 Store 归属见[Workspace 与 Registry 设计](Workspace与Registry设计.md)
...
- 定义 Canonical Declared View、Runtime Context、Resolve 和 Probe 的协作；
- 定义 Provider 内部端口、Safe Probe、安全和错误分类；
- 定义 Observation、vantage、freshness、Store 能力和 Route evidence 聚合；
- 保持一条从规范声明视图到公共结果的连续技术叙事。

本文不负责：

- 重复定义用户维护的 YAML、命令、flags、JSON 和退出码；
- 定义 Locus Home、Registry Source、项目目录发现或 Scope 组合策略；
...
flowchart LR
    ...
    WORKSPACE["Workspace<br/>Discover / Load / Compose"] --> VIEW["Canonical Declared View"]
    VIEW --> CORE
...
- **Workspace** 选择 Registry Source，并把 Scope 组合成经过严格校验的 Declared View。
- **Declared View** 保存引用已规范化的已知事实及其 provenance。
- **Runtime Context** 保存本次调用才成立的 actor、vantage 和 Provider 明确需要的现场值。
- **Provider Registry** 将稳定 Provider name 映射到机制专属 adapter。
- **Observation Store** 只保存 canonical Link 在特定现场下的实际测量。
- **Locus Core** 编排上述输入；入口层不复制领域模型或 evidence 规则。

Registry Source、Locus Home 和本地 SQLite 的归属由 [Workspace 与 Registry 设计](Workspace与Registry设计.md)定义；具体文件布局和数据库 schema 仍是实现细节。
...
Registry Source、Scope 发现、import DAG、项目目录关联和 Effective View 组装由 [Workspace 与 Registry 设计](Workspace与Registry设计.md)定义。Core 接收的 Canonical Declared View 必须已经满足：

- 每个输入完成严格解码和完整静态校验；
- canonical identity 在当前 Workspace 内唯一；
- import 只增加 namespaced declarations，不修改 imported object；
- Link `from/to` 已指向 canonical Entity；
- Route step 已指向 canonical Link；
- Binding 保留 role 到 canonical Entity 的解释关系；
- 每个声明保留 Scope、Source 和组合 provenance。
...
Workspace 发现和组合、Core 处理链以及未来执行层分别维护各自的权威设计；跨层规则只在所属文档完整定义，其他位置使用链接。
