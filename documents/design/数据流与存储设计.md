...
error       ← 稳定失败类别；不保留任意 stdout/stderr
```
...
- stdout/stderr 是不受信输入，不进入 error、evidence 或日志；当前只保留操作名和稳定失败类别，未来只能增加 Provider 明确解析并列入白名单的非敏感字段；
- 截断和敏感词黑名单都不能证明 redaction 完整，不能作为保存任意进程输出的依据；
