...
const activeRoute = computed(() =>
  graphQuery.data.value?.routes.find(route => route.canonical_id === selectedRoute.value),
)
    ...
    <PageHeader :eyebrow="t('graph.eyebrow')" :title="t('graph.title')">
      <template #actions>
        <div class="graph-view__counts" aria-live="polite">
          <span
            ><strong>{{ graphQuery.data.value?.entities.length ?? 0 }}</strong> {{ t('graph.entities') }}</span
          >
          <span
            ><strong>{{ graphQuery.data.value?.links.length ?? 0 }}</strong> {{ t('graph.links') }}</span
          >
          <span
            ><strong>{{ graphQuery.data.value?.routes.length ?? 0 }}</strong> {{ t('graph.routes') }}</span
          >
        </div>
      </template>
    </PageHeader>
